    ...
    def fromfront(cls: type, ftype: front.UserType) -> Option["UserType"]:
        return Okay(
            cls(
                ...
                name=ftype.name,
                mbrs={
                    name: TypeIdent.name(mbr.typename)
...
class Func(Item):
    ...
    def fromfront(cls: type, ffunc: front.Func) -> "Func":
        return Okay(
            cls(
                ...
                args={
                    name: TypeIdent(mbr.typename) for name, mbr in ffunc.args.items()
                },
                locals={
                    name: TypeIdent(mbr.typename) for name, mbr in ffunc.locals.items()
                },
                frame=Nothing,
            )
        )
    ...
    def resolve(self, mod: Module) -> Result[None, str]:
        self.args = {
            name: match(ident)[
                TypeIdent.name : lambda: mod.find(ident.name), TypeIdent.type : ...
            ]
            for name, ident in self.args.items()
        }
        self.locals = {
            name: match(ident)[
                TypeIdent.name : lambda name: mod.find(name), TypeIdent.type : ...
            ]
