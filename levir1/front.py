...
levir_builtin_bool = levir_builtins.module.items["Bool"]
...
class Clause:
    ...
    def check(self, func: Func) -> CheckResult:
        ...
        if isvariant(self, (Clause._if, Clause._elif)):
            if self.cond.type.type is not levir_builtin_bool:
                reports.add(
                    CheckIssue.error(
                        self.cond.type.loc,
        ...
        reports |= match(self)[
            Clause._if,
            Clause._elif : lambda _, cond, frame: (
    ...
    def fromtree(cls: type, mod: Module, tree: Tree) -> Result["Clause", str]:
        ...
        return Okay(
            match(tree.data)[
                "if",
                "elif" : lambda: Clause._if(loc, cond, frame),
