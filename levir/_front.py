...
class TypeIdent:
    ...
    def __new__(cls, loc: Location, obj: object, by_ref: bool):  # -> 'TypeIdent':
        ...
        return match(obj)[
            ...
            ... : TypeError(
                f"{cls.__name__} cannot make from an object of "
    ...
    def fromtree(cls: type, mod: "Module", sometypetree: Tree):
        ...
        typetree = match(sometypetree.data)[
            ...
            "vartype",
            "rettype",
            "outtype" : lambda: sometypetree.children[0],
            ... : MatchError(
        ...
        return Okay(
            cls(Location.fromtree(mod.filename, sometypetree), namefrom(identtok), ref)
        )

    @classmethod
    def seqfromlisttree(
        cls: type, mod: "Module", pairstree: Tree
    ) -> Dict[str, "TypeIdent"]:
        # unwrap list if it is wrapped in mbrs or args
        if pairstree.data != "list":
            assert len(pairstree.children) <= 1, unsupported(
                mod.filename,
                pairstree,
                f"cannot make a TypeIdent "
        ...
        return Okay(
            {
                ...
                for nameident, typetree in (pair.children for pair in listtree.children)
            }
        )
    ...
    def fromfile(cls: type, file) -> Result["Module", None]:
        ...
        for deftree in deftrees:
            item = ~match(deftree.data)[
                ...
                "structdef" : lambda: Struct.fromtree(
                    mod, deftree
                ),  # Error('structdef not implemented yet'),#translate_classdef(mod, ),
                "fndef" : lambda: Func.fromtree(
                    mod, deftree
                ),  # Error('funcdef not implemented yet'),#translate_classdef(mod, ),
...
class UserType(Item):
    ...
    def resolve(self, mod: Module) -> Result[None, str]:
        self.mbrs = {name: ~ident.resolvefrom(mod) for name, ident in self.mbrs.items()}
        return Okay(None)

    @classmethod
    @strictly
    def fromtree(cls: type, mod: Module, typetree: Tree) -> Result["Type", None]:
        assert len(typetree.children) == 4, unsupported(
            mod.filename,
            typetree,
            f"this version of levir only supports "
            f" 4 attribues, found {len(typetree.children)}",
        )

        nametree, supertree, mbrstree, mthdstree = typetree.children
        ...
        return Okay(
            cls(
                ...
                namefrom(nametree),
                ~TypeIdent.seqfromlisttree(mod, mbrstreee),
            )
        )
...
class Func(Item):
    ...
    def check(self) -> CheckResult:
        ...
        if self.rettype.isref():
            reports.add(
                CheckIssue.error(
                    self.rettype.loc, f"struct references cannot be returned"
                )
            )
    ...
    def resolve(self, mod: Module) -> Result[None, str]:
        ...
        self.args = {name: ~ident.resolvefrom(mod) for name, ident in self.args.items()}
        self.locals = {
            name: ~ident.resolvefrom(mod) for name, ident in self.locals.items()
        }
    ...
    def fromtree(cls: Type, mod: Module, fntree: Tree) -> Result["Func", None]:
        assert len(fntree.children) == 5, unsupported(
...
class Mthd(Func):
    ...
    def fromtree(cls: Type, mod: Module, mthdtree: Tree) -> Result["Mthd", None]:
        assert len(mthdtree.children) == 6, unsupported(
            mod.filename, fntree, f"this funciton grammar is not yet supported"
        )
        (
            outtype,
...
class Subject:
    class var:
        ...
        @strictly
        def _fromtree(
            cls: type, mod: Module, vartree: Tree
        ) -> Result["Subject.var", str]:
            assert vartree.data == "var", unsupported(
                mod.filename,
                vartree,
    ...
    class mbrof:
        ...
        def resolve(self, mod: Module) -> Result[None, str]:
            ...
            self.mbrs = {
                name: ~ident.resolvefrom(mod) for name, ident in self.mbrs.items()
            }
            return Okay(None)

        @classmethod
        def _fromtree(cls: type, mod: Module, mbroftree: Tree):
            assert len(mbroftree.children) == 3, unsupported(
                mod.filename,
                mbroftree,
            ...
            mbrs = match(mbrspectree.data)[
                ...
                ... : MatchError(f"unable to match '{mbrspectree.data}'"),
            ]
    ...
    def fromtree(cls: type, mod: Module, tree: Tree) -> Result["BinOp", None]:
        ...
        return Okay(
            match(tree.data)[
                ...
                ... : MatchError(f"no known operation for '{tree.data}'"),
            ]
        )
...
class Expr:
    ...
    class get:
        ...
        def _fromtree(cls: type, mod: Module, gettree: Tree):
            assert len(gettree.children) in (1, 2), unsupported(
                mod.filename,
                gettree,
    ...
    class new:
        ...
        def _fromtree(cls: type, mod: Module, newtree: Tree):
            assert len(newtree.children) == 2, unsupported(
                mod.filename,
                newtree,
            ...
            return Okay(
                cls(
                    ...
                    type=~TypeIdent.fromtree(mod, typetree),
                    cntn=tuple(~Expr.fromtree(mod, tree) for tree in cntnlist.children),
                )
            )
    ...
    class litrl:
        ...
        @strictly
        def _fromtree(
            cls: type, mod: Module, litrltree: Tree
        ) -> Result["Expr.litrl", None]:
            assert len(litrltree.children) == 2, unsupported(
                mod.filename,
                litrltree,
    ...
    class binop:
        ...
        def _fromtree(cls: type, mod: Module, litrltree: Tree):
            assert len(litrltree.children) == 4, unsupported(
                mod.filename,
                litrltree,
    ...
    def fromtree(cls: type, mod: Module, exprtree: Tree):
        return match(exprtree.data)[
            ...
            ... : MatchError(
                unsupported(
                    mod.filename, exprtree, f"cannot compile unknown kind of expression"
                )
            ),
        ]
...
class Clause:
    ...
    def check(self, func: Func) -> CheckResult:
        ...
        if isvariant(self, (Clause.if_, Clause.elif_)):
        ...
        reports |= match(self)[
            Clause.if_,
            Clause.elif_ : lambda _, cond, frame: (
                ~cond.check(func) | ~frame.check(func)
            ),
            Clause.else_ : lambda _, frame: ~frame.check(func),
        ]
    ...
    def resolve(self, mod: Module) -> Result[None, str]:
        ...
        match(self)[
            ...
            Clause.else_ : lambda _, frame: ~frame.resolve(mod),
        ]
        return Okay(None)
    ...
    def fromtree(cls: type, mod: Module, tree: Tree) -> Result["Clause", str]:
        global Clause
        assert tree.data in ("if", "elif", "else"), unsupported(
            mod.filename, tree, f"unknown conditional clause kind '{tree.data}' "
        )
        ...
        return Okay(
            match(tree.data)[
                "if",
                "elif" : lambda: Clause.if_(loc, cond, frame),
                #'elif' : lambda: Clause.elif_(loc, cond, frame),
                "else" : lambda: Clause.else_(loc, frame),
                ... : MatchError(
...
class Stmt:
    class asn:
        ...
        def _fromtree(cls: type, mod: Module, asntree: Tree):
            assert len(asntree.children) in (2, 3), unsupported(
                mod.filename,
                asntree,
    ...
    class ret:
        ...
        def _fromtree(cls: type, mod: Module, rettree: Tree):
            assert len(rettree.children) in (1, 2), unsupported(
                mod.filename,
                rettree,
    ...
    class dropin:
        ...
        def fromtree(cls: type, mod: Module, tree: Tree) -> Result["Stmt.dropin", str]:
            assert len(tree.children) == 2, unsupported(
                mod.filename,
                tree,
    ...
    class cond:
        ...
        def fromtree(cls: type, mod: Module, tree: Tree) -> Result["Stmt.cond", str]:
            assert len(tree.children), unsupported(
                mod.filename,
                tree,
    ...
    def fromtree(cls: type, mod: Module, tree: Tree) -> Result["Stmt", str]:
        ...
        return Okay(
            match(tree.data)[
                ...
                ... : MatchError(f"no case for '{tree.data}' in constructing a stmt"),
            ]
        )
