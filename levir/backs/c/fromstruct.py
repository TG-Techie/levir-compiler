...
def tobody(mod:Module, sct:Struct) -> str:
    ...
    dfltblock = '{\n        ' + (
        '(),\n        '.join( f".{mbr_(mbrname, mbrtype)} = {dflt_(mbrtype)}"
            for mbrname, mbrtype in sct.mbrs.items()
        )
    ) + '(),\n    }'
    ...
    getblock = '{\n        ' + (
        ',\n        '.join( f".{mbr_(mbrname, mbrtype)} = {get_(mbrtype)}(self.{mbr_(mbrname, mbrtype)})"
            for mbrname, mbrtype in sct.mbrs.items()
        )
    ) + ',\n    }'

    dropblock = (
        ';\n    '.join( f"{drop_(mbrtype)}(self.{mbr_(mbrname, mbrtype)})"
            for mbrname, mbrtype in sct.mbrs.items()
        )
    )
