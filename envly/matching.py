    ...
    def infer(cls, obj: object) -> "Action":
        ...
        if isinstance(obj, Action):
            ...
            raise InferenceError(
                f"unable to infer action to take based on "
...
class Pattern(DataVariable, Generic[M]):
    ...
    def covers(self, to: M) -> Result[Union[None, str], str]:  # Result.okay = passes,
        ...
        if isinstance(to, Enum) and to._enum_cls_ not in self:
            ...
            if Case.default in self:
                ...
                covered = set(
                    each
                    for each in self.cases()
                    if isinstance(each, (EnumVariant, EnumVariantClass))
                )
                varnt_cases = set(
                    Case.infer(varnt) for varnt in to._enum_varnts_.values()
                )
            ...
            if (
                Case.is_a(bool) in self
                or Case.is_this(tru) in self
                and Case.is_this(fls) in self
            ):
                return Okay("bool or True and False")
            else:
                return Error(
                    f"does not cover {repr(to)}, must "
            ...
            if Case.default in self:
                ...
                return Error(
                    f"requires a default case, " "ex: [...: <some value/lambda/action>]"
                )
    ...
    def __class_getitem__(cls, slices: PatternSyntax) -> Result["Pattern", str]:
        if isinstance(slices, tuple):
            ...
            raise TypeError(f"must have more than one case, " f"found {repr(slice)}")
        ...
        for thing in slices:
            if isinstance(thing, (list, set, types.GeneratorType)):
                ...
                if thing.step is None:
                    ...
                    raise TypeError(
                        f"three parameter cases not supported, "
...
def test(print):
    ...
    assert Case.equals(5) == Case.infer(
        5
    ), "manual intervention required, fix inference"
    assert Case.is_this(True) == Case.infer(
        True
    ), "manual intervention required, fix inference"
    assert Case.is_this(False) == Case.infer(
        False
    ), "manual intervention required, fix inference"
    assert Case.is_a(bool) == Case.infer(
        bool
    ), "manual intervention required, fix inference"
    assert Case.is_in(range(5)) == Case.infer(
        range(5)
    ), "manual intervention required, fix inference"
    assert Case.default == Case.infer(
        ...
    ), "manual intervention required, fix inference"
    ...
    assert str(range(5)) == case_to_str(
        Case.is_in(range(5))
    ), "manual intervention required"
    assert "all(...)" == case_to_str(
        Case.check(all)
    ), f"manual intervention required, got {case_to_str(Case.check(print))}"
    assert f"Case.subclasses(cls={int})" == case_to_str(
        Case.subclasses(int)
    ), "manual intervention required"
    assert repr(Nothing) == case_to_str(
        Case.enum(Option.Nothing)
    ), "manual intervention required"
    ...
    try:
        ...
        print(
            "passed: should cover `color.red` and `color.green` but not `color.blue` or `color.custom` is it?:"
        )
        print("    ", repr(err))
    ...
    assert (
        match(vars.foo).against(p) is vars.foo
    ), "match(vars.foo) did not match vars.foo"
    assert match(vars.bar(7)).against(p) == 7, "match(vars.foo) did not match vars.foo"
    assert match(vars.baz(8, 9)).against(p) == (
        8,
        9,
    ), "match(vars.foo) did not match vars.foo"
