...
class Enum:
    def __new__(cls, *_, **__):
        raise TypeError(
            f"must specify enum variant to init {cls}, like "
...
def enumcls_from_cls(src_cls):
    ...
    if len(src_cls.__bases__) > 1:
        raise TypeError(
            f"Enums may only inherit from one Generic class, " f"found multiple {bases}"
        )
    ...
    if base is not object and not issubclass(base, typing.Generic):
        raise TypeError(
            f"Enums cannot inherit from non-Generic Base classes, "
...
def varnt_from_cls(
    cls: type, enum_cls: EnumClass
) -> Union[EnumVariantClass, EnumSingleVariant]:
    assert cls.__bases__ == (object,), (
        f"enum variants can only inherit from " f"object, found {cls.__bases__}"
    )
    ...
    if not hasattr(cls, "__annotations__"):
        ...
        var_cls = EnumVariantClass(name, (EnumDataVariant, enum_cls), {**cls.__dict__})
...
def test(print):
    ...
    for inst, cls in cases.items():
        assert isinstance(inst, cls), f"{inst} should an isinstance of {cls}"
    ...
    for cls, supercls in cases.items():
        assert issubclass(cls, supercls), f"{cls} should inherit from {supercls}"
